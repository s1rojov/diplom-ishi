...
<template>
  <div class="px-7 flex flex-col gap-3 pt-3">
    <div
      class="border bg-white border-neutral-400/40 py-3 px-4 rounded-2xl flex items-center justify-between"
    >
      <div>
        <div
          class="text-zinc-950 text-lg font-semibold leading-relaxed tracking-wide"
        >
          Rahbariyat
          <sup>3</sup>
        </div>
        <span
          class="text-neutral-400 text-xs font-normal leading-normal tracking-wide"
          >Ushbu bo’limda katalog yaratishingiz</span
        >
      </div>
      <div class="flex items-center gap-2">
        <BaseIcon
          ...
          :class="[isItemList ? 'text-[#C49E5E]' : 'text-neutral-400']"
    ...
    <NotificationModal
      :isOpenModal="isOpen"
      content="O'chirmoqchimisiz?"
      @selectedBtnClicked="test"
    >
      <template #closeBtn>
        <BaseIcon
          name="close"
